...
var EqptChangeDialog = (function (_super) {
    __extends(EqptChangeDialog, _super);
    function EqptChangeDialog() {
        _super.call(this);
        this.textList = new egret.gui.List();
        this.dp = new egret.gui.ArrayCollection();
    }
    EqptChangeDialog.prototype.createChildren = function () {
    ...
    EqptChangeDialog.prototype.clickItem = function (e) {
        var item = e.item;
        var operateObj = Main.player;
        if (Number(Main.player.m_operateObjid) > 0) {
            operateObj = Main.player.m_heros.heros[Main.player.m_operateObjid];
        }
        if (!operateObj || !(operateObj.id > 0))
            return;
        if (item.tpl["ejob"] != operateObj.job && Number(item.tpl["ejob"]) != 0)
            return;
        var url = "http://" + GameManager.server["ip"] + ":" + GameManager.server["port"];
        var param = "tagid=" + operateObj.id + "&objid=" + item["id"] + "&type=0x0005&sid=" + GameManager.SESSION_ID;
        HttpMessage.sendMsg(url, param);
        this.hide();
    };
    EqptChangeDialog.prototype.setVis = function (b) {
        if (b === void 0) { b = false; }
        if (this.m_UI) {
    ...
    EqptChangeDialog.prototype.onTouch = function (e) {
        this.hide();
    };
    EqptChangeDialog.prototype.onUpdate = function () {
        this.dp.removeAll();
        ...
        for (var i in Main.player.m_items.equips) {
            var equip = Main.player.m_items.equips[i];
        ...
        if (Main.player.m_clickItme) {
            ...
            this.m_UI.xz_label_property._textField.textFlow = GameManager.GetEqptPropStr(Main.player.m_clickItme);
    ...
    EqptChangeDialog.prototype.show = function () {
        _super.prototype.show.call(this);
        this.onUpdate();
    };
    EqptChangeDialog.prototype.hide = function () {
        _super.prototype.hide.call(this);
    };
    return EqptChangeDialog;
})(Dialog);
EqptChangeDialog.prototype.__class__ = "EqptChangeDialog";
