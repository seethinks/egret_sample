...
var HeroEqpt = (function (_super) {
    __extends(HeroEqpt, _super);
    function HeroEqpt() {
        _super.call(this);
        this.iconAny = {};
        this.iconWarn = {};
    }
    HeroEqpt.prototype.createChildren = function () {
        ...
        for (var i = 1; i <= 10; i++) {
            var icon = this.m_UI["eqpt_icon_" + i];
    ...
    HeroEqpt.prototype.addEqptWarm = function (postion) {
        if (!this.initUI) {
            this.iconWarn[postion] = 1;
        }
        else {
            var icon = this.m_UI["eqpt_icon_" + postion];
            if (icon) {
                icon.setEqptVis(true);
            }
        }
    };
    HeroEqpt.prototype.onClick = function (e) {
        DialogManager.open(MorePropDialog, "MorePropDialog");
    };
    HeroEqpt.prototype.onClickIcon = function (e) {
        var icon = e.currentTarget;
        icon.setEqptVis(false);
        var index = Number(e.currentTarget.name);
        if (this.iconWarn[index])
            delete this.iconWarn[index];
    ...
    HeroEqpt.prototype.onUpdatePlayer = function () {
        if (!this.initUI)
            return;
    ...
    HeroEqpt.prototype.onUpdate = function () {
        var hold = Main.player.m_items.holdEq;
        for (var i in this.iconAny) {
            var icon = this.iconAny[i];
            if (icon) {
                if (this.iconWarn[i])
                    icon.setEqptVis(true);
                var equip = hold[i];
    ...
    HeroEqpt.prototype.show = function () {
        _super.prototype.show.call(this);
        this.onUpdate();
        this.onUpdatePlayer();
    };
    HeroEqpt.prototype.hide = function () {
        _super.prototype.hide.call(this);
    };
    return HeroEqpt;
})(Dialog);
HeroEqpt.prototype.__class__ = "HeroEqpt";
