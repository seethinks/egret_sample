var __extends = this.__extends || function (d, b) {
    for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p];
    function __() { this.constructor = d; }
    __.prototype = b.prototype;
    d.prototype = new __();
};
var DialogBox = (function (_super) {
    __extends(DialogBox, _super);
    function DialogBox() {
        _super.call(this);
        this.title = "界面";
        this.skinName = "skins.DialogMessageBoxSkin";
        this.height = 720;
    }
    DialogBox.prototype.partAdded = function (partName, instance) {
        _super.prototype.partAdded.call(this, partName, instance);
        if (instance == this._dialog_label) {
            this._dialog_label.text = this.title;
        }
    };
    DialogBox.prototype.setSize = function (_h) {
        if (_h === void 0) { _h = 0; }
    };
    DialogBox.prototype.setTitle = function (_s) {
        this.title = _s;
        if (this._dialog_label) {
            this._dialog_label.text = _s;
        }
    };
    return DialogBox;
})(egret.gui.SkinnableComponent);
DialogBox.prototype.__class__ = "DialogBox";
