class Heart
{
    ...
    private gameHeartTime:number = 0;

    private serverHeart:number = 0;

    private fightHeart:number = 0;

    private flyTxtHeart:number = 0;

    private msgsendTime:number = 0;

    private hornHeart:number = 0;

    private prisonTime:number = 0;
    public constructor()
    {
    ...
    private onTimer(e:egret.TimerEvent):void
    {
        ...
        if(this.serverHeart >= 113) {
            ...
            var params:string = "type=0x0040&sid = "+GameManager.SESSION_ID;
...
//            GameManager.nextFightTime --;
        ...
        if(GameManager.startGame && Main.player.m_leaveResult){
            ...
            if(result) result.updateText(Main.player.m_leaveResult);
        }
        ...
        if(this.hornHeart >= 10)
        {
            ...
            {
                var dialog:SocietyDialog = DialogManager.findDialog(SocietyDialog, "SocietyDialog");
        ...
        if(this.prisonTime >= 10)
        {
            ...
            var prison:PrisonDialog = DialogManager.findDialog(PrisonDialog, "PrisonDialog", true);
