...
class PrisonDialog extends Dialog
{
    private m_dialog:DialogBox;

    private m_UI:PrisonUI;
...
//                        if(GameManager.dataEyes)GameManager.dataEyes.pushEvent("红名贿赂", 1,
//                            {cost:10, pname:Main.player.name, sid:GameManager.server.sid});
                    }
                }, "是否花费10元宝消除100点PK值？");
                break;
            case this.m_UI._btn_zhuilu:
                ...
                GameManager.msgBox.showMsg(function(r)
                {
                    if(r)
                    {
                        ...
                        if(GameManager.dataEyes)GameManager.dataEyes.pushEvent("红名贿赂", 1,
                            {cost:Math.floor(Main.player.pk * 0.08), pname:Main.player.name, sid:GameManager.server.sid});
                    }
