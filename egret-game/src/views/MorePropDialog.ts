class MorePropDialog extends Dialog {
    ...
    public onUpdate():void
    {
        ...
        var text:string = "Lv."+Main.player.level + " "+Main.player.name+"["+GameManager.JOB_NAME[Main.player.job]+"]";
        var color:number = GameManager.getNameColor();
        var size:number = 18;
        ...
        var rate:number = Main.player.armor / (Number(Number(10*Main.player.level*Main.player.level)+Number(60*Main.player.level+240))*8);
        ...
        for(var pos in Main.player.m_items.holdEq){
            var equip:Equip = Main.player.m_items.holdEq[pos];
