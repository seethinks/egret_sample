class UIEqptSelectListTo extends egret.gui.ItemRenderer
{
    public constructor()
    {
        super();
    ...
    private onCheck(e:egret.TouchEvent):void
    {
        if(this.data)
        {
            var dialog:ArmsInheritCheck = DialogManager.findDialog(ArmsInheritCheck, "ArmsInheritCheck");

            if(dialog && dialog.showing) dialog.setIndex();
